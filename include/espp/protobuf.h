...
#define PROTO_PACK(p) \
    ...
    void Unpack(const std::vector<uint8_t>& buffer) { \
        assert(!_is_unpacked); p##__unpack(nullptr, buffer.size(), buffer.data()); _is_unpacked = true; \
    }

#define PROTO(x, p) struct PROTO_NAME(x): protected x { \
...
#define PROTO_NESTED(x, p, xx) struct xx: protected x { \
    bool _is_unpacked = false; \
    PROTO_CONSTRUCT_NESTED(x, p, xx) \
    x* ptr() {return this;}

#define PROTO_STR(attr) \
    void set_##attr(const std::string& value) { attr = const_cast<char*>(value.c_str()); } \
...
#define PROTO_MSG(attr, cls) \
    cls _internal##attr; \
    void set_##attr(cls v) { _internal##attr = v; attr = _internal##attr.ptr(); } \
